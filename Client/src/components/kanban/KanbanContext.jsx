...
const TAB_GROUP_STORAGE_PREFIX = 'nodelit:tabgroups:';
...
function persistCollapsedGroups(workspaceID, groupIDs) {
    try {
        localStorage.setItem(`${TAB_GROUP_STORAGE_PREFIX}${workspaceID}`, JSON.stringify([...groupIDs]));
    } catch {
        return;
    }
}
...
export function KanbanProvider({ children }) {
    ...
    useEffect(() => {
        ...
        try {
            const stored = localStorage.getItem(`${TAB_GROUP_STORAGE_PREFIX}${workspaceID}`);
            setCollapsedGroups(new Set(stored ? JSON.parse(stored) : []));
        } catch {
            setCollapsedGroups(new Set());
        }
    }, [workspaceID]);
    ...
    const toggleGroup = useCallback(groupID => {
        setCollapsedGroups(previous => {
            ...
            persistCollapsedGroups(workspaceRef.current, next);
    ...
    return (
        <KanbanContext.Provider value={{ boardData, setBoardData, applyDelta, workspaceID, loading, error, refresh, memberRole, canEdit: EDIT_ROLES.has(memberRole), expandedChecklists, toggleChecklist, collapsedGroups, toggleGroup }}>
            {children}
